...
将公网 Docker 镜像与 Helm Chart 搬运（镜像）到私有 registry（阿里云 ACR / 腾讯云 TCR）。没有可构建的应用代码——核心逻辑全部是三个 GitHub Actions workflow 里的 bash 脚本。
...
- `config.yaml`（根目录）— 搬运行为开关统一配置，扁平小写下划线 key（`push_images`/`push_charts`/`keep_image_namespace`/`keep_chart_namespace`），全部默认 `true`。三个 workflow 运行时用 `yq '.key' config.yaml` 读原始值，再用 `[ "$X" = "false" ] || X="true"` 归一化——只有显式 `false` 才是 false，缺失/注释掉/其余值一律回退默认 `true`。**不能用 `yq '.key // "默认"'`：`//` 对 `false` 也会取默认值，会把 `false` 吞成 `true`。** docker 流程只读 `push_images`/`keep_image_namespace`。
...
- `docs/superpowers/specs/` — 设计文档，改动搬运逻辑前应先读。

## 关键约定

- 三个 workflow 都由 `workflow_dispatch`（手动，无 inputs）或 `push: master` 触发。搬运行为开关不再走 workflow input，而是运行时从根目录 `config.yaml` 用 `yq` 读取后归一化（显式 `false` 才生效，否则默认 `true`）——改默认行为就改 config.yaml 或脚本里的归一化逻辑。
