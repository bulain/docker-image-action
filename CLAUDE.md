...
- `config.yaml`（根目录）— 搬运行为开关统一配置，扁平小写下划线 key（`push_images`/`push_charts`/`keep_image_namespace`/`keep_image_original_tag`/`keep_chart_namespace`）。三个 workflow 运行时用 `yq '.key // "默认"' config.yaml` 读取（带 fallback，注释掉某行即回退默认）。docker 流程只读 `push_images`/`keep_image_namespace`。
