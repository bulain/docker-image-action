...
# repo: git 仓库地址  path: chart 子路径  ref: git 分支/tag/commit
charts:
...
git 源没有 OCI 路径可继承。`KEEP_CHART_NAMESPACE=true` 时用 chart 名
（取 `path` 的 basename，如 `ps-operator`）作子路径 → `oci://$TCR/$TCR_NS/ps-operator`；
关闭则直推 `oci://$TCR/$TCR_NS`。
